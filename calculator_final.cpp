...
digit * addNumbers(digit * left, digit * right){ // this function makes numbers in order.
 ...
 int countR = 0;
 digit *copyLeft = left;
 digit *copyRight = right;
 while (copyLeft != NULL) {
  ++countL;
  copyLeft = copyLeft->next;
 }
 while (copyRight != NULL) {
  ++countR;
  copyRight = copyRight->next;
 }
 int result1 = 0;
 digit *head;
 digit *result3 = new digit;
 digit *result4 = new digit;
 head = NULL;
 if (countL == countR) {
  ...
  while (left != nullptr || right != nullptr) {
   digit *temp = new digit;
   int result = 0;
   result = left->data + right->data + result1;
   if (result >= 10) {
    result = result - 10;
    result1 = 1;
   }
